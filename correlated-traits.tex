...
note that by e.g. taking the Cholesky decomposition of $\Sigma$ we have a matrix $A$ such that $X = A Z$,
with $Z$ independent N(0,1).
Then if $W$ is the matrix whose rows are given by $w$,
\begin{align}
  \tilde X = (I-W) A Z,
\end{align}
and hence
\begin{align}
  \cov[\tilde X, \tilde X] = (I-W)A A^T (I-W^T) = (I-W) \Sigma (I-W^T) .
\end{align}
