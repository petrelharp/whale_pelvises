...
\begin{align}
    ...
    &= k_s \left\{ 2\left( \sum_{e \in (s \to t) \cap (u \to v) } (-1)^{\epsilon_e^{s \to t}+\epsilon_e^{u \to v}}\tau'_e \right)^2 \right\} \\
    &= k_s \left\{ 2\left( \sum_{e \in (s \to t) \cap (u \to v) } \tau'_e \right)^2 \right\} .
\end{align}
...
%   ( 0.484 ) / ( 0.484 + (1.965*0.805) + 0.926 )
\[
\frac{\beta_P}{ \delta_P \sigma_L + \beta_P + \sigma_P } = 0.16 .
\]
For ribs, this is
%   0.064 / ( 0.064 + (1.965*0.558) + 0.521 )
\[
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics{posterior-correlations}
  \end{center}
