...
\end{align*}
with the $Y_e$ a vector of independent Gaussians with variance equal to $\tau'_e := (\sigma^2_S + \gamma_P C_e) \tau_e$.
The parameters only enter through these rescaled branch lengths.
Now 
\begin{align}
    \frac{ \|S_t - S_s\|^2 }{ \sum_{e \in s \to t} \tau'_e } \sim \chi^2_{k_s} ,
\end{align}
which for large $k_s$ we can approximate as Gaussian,
with parameters
\begin{align*}
...
\begin{align}
    \cov[ \|S_t - S_u\|^2, \|S_s - S_u\|^2 ] &= \sum_{k=1}^{k_s} \cov\left[ (S_t-S_u)_k^2, (S_s-S_u)_k^2 \right] \\
                                             &= \sum_{k=1}^{k_s} \var\left[ \left( \sum_{e \in (u \to t) \cap (u \to s)} Y_e \right)^2 \right] \\
