...
stopifnot( all( sapply( pargrids[-1], function (x) all.equal(pargrids[[1]], x) ) ) )
pargrid <- pargrids[[1]]
...
if (FALSE) {
    # for merging together two grids...
    ...
    for (type in c("pelvic","sub.pelvic","rib")) {
        ...
        for (this.ks in sort(unique(pargrid$ks))) {
            ...
            for (k in sample(ncol(this.sgrids),30)) {
                ...
                dim(z) <- c( length(sigma2S.vals), length(gammaP.vals) )
                contour( sigma2S.vals, gammaP.vals, z, add=TRUE, col=adjustcolor(this.ks-12,.2) )
            }
        }
    }
}
