...
our model is that $X_t = X_0 + A_t Z$, with $Z$ independent standard Gaussians and $A_t$ the matrix given here:
...
\begin{bmatrix}
    ...
    \sigma_L \delta_T \sqrt{t}  &  \beta_T \sqrt{t}  & 0  &   0 \\
    \sigma_L \delta_R \sqrt{t}  &  \beta_R \sqrt{t}  & \sigma_R \sqrt{t}  &   0 \\
    \sigma_L \delta_R \sqrt{t}  &  \beta_R \sqrt{t}  & \sigma_R \sqrt{t}  &   0 \\
    \sigma_L \delta_P \sqrt{t}  &  \beta_P \sqrt{t}  & 0  &   \sigma_P\sqrt{t}  \\
    \sigma_L \delta_P \sqrt{t}  &  \beta_P \sqrt{t}  & 0  &   \sigma_P\sqrt{t}  
\end{bmatrix}
\begin{bmatrix}
    Z_1 \\ Z_2 \\ Z_3 \\ Z_4
...
A simple computational way around this is to let $P$ be the projection matrix into the columns space of $C$,
and to transform both the data and the covariance by this, letting $Y = P \widetilde X$ and $Q = P C$.
Since weights $M$ assign zero influence to missing values in the data,
we can ignore these henceforth.
(Another choice would to be to let $P$ be the matrix that drops one individual; 
...
Now, we place priors on the parameters \ldots
